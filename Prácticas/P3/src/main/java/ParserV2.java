...
public class ParserV2 {
    ...
    public ParserV2(Lexer analizadorLexico) {
        this.analizadorLexico = analizadorLexico;
    }

    private void obtenerSiguienteToken() throws IOException {
        int claseLexica = analizadorLexico.yylex();
        String lexema = analizadorLexico.yytext();
        tokenActual = new Token(claseLexica, lexema);
    }

    public void eat(int claseLexicaEsperada) throws IOException, ParserException {
        if (tokenActual.getClase() == claseLexicaEsperada) {
            obtenerSiguienteToken();
        } else {
            throw new ParserException("Token inesperado: se esperaba '" + obtenerNombreToken(claseLexicaEsperada) +
                    "', pero se encontró '" + obtenerNombreToken(tokenActual.getClase()) + "' con lexema '" + tokenActual.getLexema() + "' en la línea " + analizadorLexico.getLine());
        }
    }
    ...
    public void declaraciones() throws IOException, ParserException {
        if (tokenActual.getClase() == ClaseLexica.INT || tokenActual.getClase() == ClaseLexica.FLOAT) {
            declaracion();
            declaraciones_();
        }
    }
    ...
    public void tipo() throws IOException, ParserException {
        if (tokenActual.getClase() == ClaseLexica.INT) {
            eat(ClaseLexica.INT);
        } else if (tokenActual.getClase() == ClaseLexica.FLOAT) {
    ...
    public void sentencias() throws IOException, ParserException {
        if (tokenActual.getClase() == ClaseLexica.ID || tokenActual.getClase() == ClaseLexica.IF || tokenActual.getClase() == ClaseLexica.WHILE) {
            sentencia();
            sentencias_();
        }
    }
    ...
    public void expresion_suma_() throws IOException, ParserException {
        if (tokenActual.getClase() == ClaseLexica.SUMA) {
            eat(ClaseLexica.SUMA);
            expresion_mult();
            expresion_suma_();
        } else if (tokenActual.getClase() == ClaseLexica.RESTA) {
    ...
    public void expresion_mult_() throws IOException, ParserException {
        if (tokenActual.getClase() == ClaseLexica.MULT) {
            eat(ClaseLexica.MULT);
            termino();
            expresion_mult_();
        } else if (tokenActual.getClase() == ClaseLexica.DIV) {
            eat(ClaseLexica.DIV);
            termino();
            expresion_mult_();
        }
    ...
    public void termino() throws IOException, ParserException {
        if (tokenActual.getClase() == ClaseLexica.ID) {
            eat(ClaseLexica.ID);
        } else if (tokenActual.getClase() == ClaseLexica.NUMERO) {
            eat(ClaseLexica.NUMERO);
        } else if (tokenActual.getClase() == ClaseLexica.LPAR) {
            eat(ClaseLexica.LPAR);
            expresion();
            eat(ClaseLexica.RPAR);
        } else {
